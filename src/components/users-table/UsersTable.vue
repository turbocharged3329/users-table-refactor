...
const TABLE_COLUMNS: UTableColumn[] = [
  {
    title: 'ID',
    sortValue: 'id',
    sortable: true,
...
async function loadUsers() {
  ...
  try {
    ...
    error.value = 'Ошибка загрузки данных: ' + getErrorTextMessage(err)
    console.error(err)
  }
}
...
function deleteUserHandler(userId: UserId) {
...
function sortHandler(sortValue: UTableColumnSortValue) {
  if (!sortValue) {
    return
  }

  sortBy(sortValue as keyof User)
}

async function toggleUserStatus(userId: UserId) {
  try {
